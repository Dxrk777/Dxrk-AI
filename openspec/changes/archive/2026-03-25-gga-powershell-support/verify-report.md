...
| Shim propagates exit code | ✅ Implemented | `dxrk.ps1` line 12: `exit $LASTEXITCODE` |
| Shim surfaces clear error if git not found | ✅ Implemented | `dxrk.ps1` lines 2–4: `Write-Error` + `exit 1` |
| Shim surfaces clear error if bash.exe not found | ✅ Implemented | `dxrk.ps1` lines 7–10: `Write-Error` + `exit 1` |
| `RuntimeBinDir(homeDir)` returns `~/.local/share/dxrk/bin` | ✅ Implemented | `runtime.go:17–19` |
| `RuntimePS1Path(homeDir)` returns `RuntimeBinDir + "/dxrk.ps1"` | ✅ Implemented | `runtime.go:27–29` |
