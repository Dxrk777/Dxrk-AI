...
- **DxrkMemory**: Read `sdd/{change-name}/explore` (optional) and `sdd-init/{project}` (optional). Save artifact as `sdd/{change-name}/proposal`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to DxrkMemory AND write to filesystem. Retrieve dependencies from DxrkMemory (primary) with filesystem fallback.
...
**IF mode is `DxrkMemory` or `none`:** Do NOT create any `openspec/` directories. Skip this step.
...
**IF mode is `DxrkMemory`:** Existing context was already retrieved from DxrkMemory in the Persistence Contract. Skip filesystem reads.
...
**Change**: {change-name}
**Location**: `openspec/changes/{change-name}/proposal.md` (openspec/hybrid) | DxrkMemory `sdd/{change-name}/proposal` (DxrkMemory) | inline (none)
