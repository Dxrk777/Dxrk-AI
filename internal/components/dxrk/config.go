...
// PostInstallMessages returns user guidance after Dxrk setup.
func PostInstallMessages() []string {
	return []string{
		"Run `dxrk install` in any git project to set up the pre-commit hook",
		"Copy `~/.config/dxrk/AGENTS.md` to your project root and customize it",
	}
}

// ProviderForAgents auto-detects the Dxrk provider based on selected agents.
...
// BuildConfig generates the shell-sourced Dxrk global config content.
...
// ConfigPath returns the Dxrk global config file path.
func ConfigPath(homeDir string) string {
	return filepath.Join(homeDir, ".config", "dxrk", "config")
}

// AgentsTemplatePath returns the starter AGENTS.md template path.
func AgentsTemplatePath(homeDir string) string {
	return filepath.Join(homeDir, ".config", "dxrk", "AGENTS.md")
}

// Inject writes the Dxrk global config and starter AGENTS.md template.
...
func Inject(homeDir string, agentIDs []model.AgentID) (InjectionResult, error) {
	...
	if err != nil {
		return InjectionResult{}, fmt.Errorf("write dxrk config: %w", err)
	}

	agentsContent, err := assets.Read("dxrk/AGENTS.md")
	if err != nil {
		return InjectionResult{}, fmt.Errorf("read embedded dxrk AGENTS.md template: %w", err)
	}

	agentsPath := AgentsTemplatePath(homeDir)
	agentsResult, err := filemerge.WriteFileAtomic(agentsPath, []byte(agentsContent), 0o644)
	if err != nil {
		return InjectionResult{}, fmt.Errorf("write dxrk AGENTS.md template: %w", err)
	}
