...
func TestConfigPath(t *testing.T) {
	...
	expected := filepath.Join("/home/testuser", ".config", "dxrk", "config")
	if path != expected {
...
func TestAgentsTemplatePath(t *testing.T) {
	...
	expected := filepath.Join("/home/testuser", ".config", "dxrk", "AGENTS.md")
	if path != expected {
...
func TestFilesWritten(t *testing.T) {
	result := InjectionResult{
		ConfigFile: "/home/user/.config/dxrk/config",
		AgentsFile: "/home/user/.config/dxrk/AGENTS.md",
	}
...
func TestPostInstallMessages(t *testing.T) {
	...
	if !strings.Contains(msgs[0], "dxrk install") {
		t.Error("first message should mention dxrk install")
	}
