...
// Example: <!-- Dxrk-AI:custom-agent:my-skill -->
const markerFormat = "<!-- Dxrk-AI:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /Dxrk-AI:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /Dxrk-AI:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- Dxrk-AI:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- Dxrk-AI:custom-agent:"
	suffix := " -->"
