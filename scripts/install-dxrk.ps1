...
<#
.SYNOPSIS
    dxrk — Install Script for Windows
    One command to configure any AI coding agent on any OS.

.DESCRIPTION
    Downloads and installs the dxrk binary for Windows.
    Supports installation via Go or pre-built binary from GitHub Releases.

.EXAMPLE
    # Run directly:
    irm https://raw.githubusercontent.com/Dxrk777/Dxrk-Hex/main/scripts/install.ps1 | iex

    # Or download and run:
    Invoke-WebRequest -Uri https://raw.githubusercontent.com/Dxrk777/Dxrk-Hex/main/scripts/install.ps1 -OutFile install.ps1
    .\install.ps1
...
function Get-LatestVersion {
    ...
    try {
        $response = Invoke-RestMethod -Uri $url -Headers @{ "User-Agent" = "dxrk-installer" }
    } catch {
...
function Install-ViaBinary {
    ...
    $tmpDir = Join-Path $env:TEMP "dxrk-install-$(Get-Random)"
    ...
    try {
        ...
        if (-not $installDir) {
            $installDir = Join-Path $env:LOCALAPPDATA "dxrk\bin"
        }
...
function Test-Installation {
    ...
    $locations = @(
        (Join-Path $env:LOCALAPPDATA "dxrk\bin\$BINARY_NAME.exe")
    )
    if ($gopath) {
