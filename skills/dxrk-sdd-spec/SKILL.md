...
- **DxrkMemory**: Read `sdd/{change-name}/proposal` (required). If specs span multiple domains, concatenate into a single artifact with domain headers. Save as `sdd/{change-name}/spec`.
- **openspec**: Read and follow `skills/_shared/openspec-convention.md`.
- **hybrid**: Follow BOTH conventions — persist to DxrkMemory (single concatenated artifact) AND write domain files to filesystem.
...
**IF mode is `DxrkMemory`:** Existing specs were already retrieved from DxrkMemory in the Persistence Contract. Skip filesystem reads.
...
**IF mode is `DxrkMemory` or `none`:** Do NOT create any `openspec/` directories or files. Compose the spec content in memory — you will persist it in Step 5.
