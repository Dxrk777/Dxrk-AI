...
name: skill-registry
description: >
  Create or update the skill registry for the current project. Scans user skills and project conventions, writes .dxrk/skill-registry.md, and saves to engram if available.
...
Create the `.dxrk/` directory in the project root if it doesn't exist, then write:

```
.dxrk/skill-registry.md
```
...
**Location**: .dxrk/skill-registry.md
...
- ALWAYS write `.dxrk/skill-registry.md` regardless of any SDD persistence mode
...
- Add `.dxrk/` to the project's `.gitignore` if it exists and `.atl` is not already listed
