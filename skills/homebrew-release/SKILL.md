...
description: >
  Release workflow for Gentleman-Programming homebrew-tap projects (GGA, Dxrk.Dots).
...
- After creating a git tag in GGA or Dxrk.Dots repos

## Supported Projects

| Project | Repo | Formula | Tag Format | Type |
|---------|------|---------|------------|------|
| GGA | `dxrk-guardian` | `gga.rb` | `V{version}` (e.g., `V2.6.2`) | Tarball (builds from source) |
| Dxrk.Dots | `Dxrk.Dots` | `dxrk-dots.rb` | `v{version}` (e.g., `v2.5.1`) | Pre-built binaries |

---

## Dxrk.Dots Release Process (Pre-built Binaries)
...
Update `homebrew-tap/Formula/dxrk-dots.rb`:

```ruby
class GentlemanDots < Formula
  desc "Interactive TUI installer for Dxrk.Dots development environment"
  homepage "https://github.com/Dxrk777/Dxrk.Dots"
  version "{VERSION}"
  license "MIT"

  on_macos do
    on_arm do
      url "https://github.com/Dxrk777/Dxrk.Dots/releases/download/v#{version}/dxrk-installer-darwin-arm64"
      sha256 "{SHA256_DARWIN_ARM64}"
    end
    on_intel do
      url "https://github.com/Dxrk777/Dxrk.Dots/releases/download/v#{version}/dxrk-installer-darwin-amd64"
  ...
  on_linux do
    on_intel do
      url "https://github.com/Dxrk777/Dxrk.Dots/releases/download/v#{version}/dxrk-installer-linux-amd64"
  ...
  def install
    if OS.mac? && Hardware::CPU.arm?
      bin.install "dxrk-installer-darwin-arm64" => "dxrk-dots"
    elsif OS.mac? && Hardware::CPU.intel?
      bin.install "dxrk-installer-darwin-amd64" => "dxrk-dots"
    elsif OS.linux? && Hardware::CPU.intel?
      bin.install "dxrk-installer-linux-amd64" => "dxrk-dots"
    end
  end

  test do
    system "#{bin}/dxrk-dots", "--help"
  end
end
```
...
```bash
# In Dxrk.Dots repo
git add homebrew-tap/Formula/dxrk-dots.rb
...
cp {path-to}/Dxrk.Dots/homebrew-tap/Formula/dxrk-dots.rb /tmp/homebrew-tap/Formula/
cd /tmp/homebrew-tap
git add -A
git commit -m "chore: bump dxrk-dots to v{VERSION}"
git push origin main
```
...
```bash
curl -sL https://github.com/Dxrk777/dxrk-guardian/archive/refs/tags/V{VERSION}.tar.gz | shasum -a 256
```
...
```ruby
url "https://github.com/Dxrk777/dxrk-guardian/archive/refs/tags/V{VERSION}.tar.gz"
sha256 "{NEW_SHA256}"
version "{VERSION}"
```
...
- [ ] Formula updated in Dxrk.Dots repo
